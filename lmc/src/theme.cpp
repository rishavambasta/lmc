...
const ThemeData lmcTheme::loadTheme(const QString &path) {
    ...
    QString pathTemp = "/home/rishav/prog/lmc/lmc/src/resources/themes/Classic";
    //themeData.themePath = path;
    themeData.themePath = pathTemp;

    file.setFileName(pathTemp + "/main.css");
    ...
    file.setFileName(pathTemp + "/NextStatus.html");
	if(!file.open(QIODevice::ReadOnly))
		return loadTheme(defTheme);
	themeData.sysNextMsg = QString(file.readAll().constData());
	file.close();

    file.setFileName(pathTemp + "/Request.html");
	if(!file.open(QIODevice::ReadOnly))
		return loadTheme(defTheme);
